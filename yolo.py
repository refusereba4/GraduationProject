...
class YOLO(object):
    _defaults = {
        "model_path": 'Epoch99-Total_Loss5.0547-Val_Loss6.0223.pth',
        "classes_path": 'model_data/fruitlabel_classes.txt',
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image, rate):
        print('detect_image rate:{}'.format(rate))
        resultList = []
        ...
        for i, c in enumerate(top_label):
            ...
            # resultList.append(predicted_class+'\t\t'+str(round(score,2))+'\t\t'+str(top)+','+str(left)+'\t\t'+str(bottom)+','+str(right))
            resultList.append(
                [predicted_class, str(round(score, 2)), str(top) + ',' + str(left), str(bottom) + ',' + str(right)])
    ...
    def detect_imagevideo(self, image):
        resultList = []
        image_shape = np.array(np.shape(image)[0:2])
